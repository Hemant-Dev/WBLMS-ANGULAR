...
export async function CreateEmployeeAsync(data: EmployeeModel):
...
export async function UpdateEmployeeAsync(data: EmployeeModel): Promise<ResponseModel> {
  let result: ResponseModel = {
    error: '',
    data: undefined,
    message: '',
...
export async function GetEmployeeAsync(
  ...
  await axios.post(api_url + `/paginated?page=${page}&pageSize=${pageSize}&sortColumn=${sortColumn}&sortOrder=${sortOrder}`, data)
    .then((response) => {
